...
<body class="header-fixed">
	...
	<div class="wrapper">
		...
		<div class="content-md margin-bottom-30">
			<div class="container">
				...
				@endif
				@if(Cart::count()>0)
				<h2 class="alert alert-info">{{Cart::count()}} Item(s) in shopping cart</h2>
								...
								<table class="table table-striped">
									...
									@foreach(Cart::content() as $items)
										<tr>
											...
											<a href="{{route('pages.show',$items->model->slug)}}"><img class="img-responsive" src="{{asset('assets/img/products/'.$items->model->slug.'.webp')}}" alt=""></a>
												<div class="product-it-in">
												<a href="{{route('pages.show',$items->model->slug)}}"><h3>{{$items->model->name}}</h3></a>
													<span>{{$items->model->description}}</span>
												</div>
											</td>
										<td><a href="{{route('pages.show',$items->model->slug)}}">{{$items->model->price}}</a></td>
											<td>
											...
											<td>
												<form action="{{route('cart.destroy',$items->rowId)}}" method="POST">
													{{ csrf_field() }}
													{{method_field('DELETE')}}
												<button type="submit" class="close"><span>&times;</span><span class="sr-only">Close</span></button>
												</form>
											</td>
	...
	<script src="{{asset('assets/js/forms/page_login.js')}}"></script>
